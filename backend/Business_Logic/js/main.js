...
Autodesk.Viewing.Initializer(options, () => {
    ...
    // Set up event listener to handle file selection and model loading
    document.querySelectorAll(".file-link").forEach(link => {
        link.addEventListener("click", async (e) => {
            ...
            if (fileType === 'obj' || fileType === 'dwg' || fileType === 'stl') {
                // Hide the image container and show the Forge viewer
                const imageContainer = document.getElementById('imageContainer');
                imageContainer.style.visibility = 'hidden';  // Hide the image container
                const forgeViewer = document.getElementById('forgeViewer');
                forgeViewer.style.visibility = 'visible';  // Show the Forge viewer

                // Start the translation process before loading the model
                if (urn) {
                    try {
